...
def predict(model,
    ...
    with torch.no_grad():
        ...
        for i, (imgs, lidar_fov, masks, sparse_depth, k_nn_indices, sparse_depth_gt, img_names) in enumerate(data_loader_val):
...
if __name__ == "__main__":
    ...
    imgs_root_val = os.path.join(os.path.dirname(os.path.abspath(__file__)), config_kitti.TEST_DIR, "imgs/2011_09_26/val/")

    data_depth_velodyne_root_val = os.path.join(os.path.dirname(os.path.abspath(__file__)), config_kitti.TEST_DIR, "data_depth_velodyne/val/")
    
    data_depth_annotated_root_val = os.path.join(os.path.dirname(os.path.abspath(__file__)), config_kitti.TEST_DIR, "data_depth_annotated/val/")
    torch.cuda.empty_cache()
