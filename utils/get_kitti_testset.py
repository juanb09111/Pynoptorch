...
def get_depth_data(imgs_root, depth_velodyne_folder, sync_folders, imgs_list):
    ...
    for folder in sync_folders:
        ...
        if folder in directories:
            # add png files under data_depth_velodyne + folder
            depth_velo_list = depth_velo_list + \
                getListOfDepthData(data_depth_velodyne + folder)
            ...
            depth_img_filenames = [
                s for s in depth_velo_list if folder in s]
    ...
    return depth_velo_list, source_imgs, lidar_file_list


def get_file_lists(imgs_root, data_depth_velodyne_root, n_samples=None):
    
    imgs, sync_folders = getListOfImgs(imgs_root)
    depth_velo_list, source_imgs, lidar_file_list = get_depth_data(
        imgs_root, data_depth_velodyne_root, sync_folders, imgs)

    if n_samples is None:
        return np.sort(source_imgs), np.sort(depth_velo_list), np.sort(lidar_file_list)
    
    else:
        
        return np.sort(source_imgs)[:n_samples], np.sort(depth_velo_list)[:n_samples], np.sort(lidar_file_list)[:n_samples]


class kittiDataset(torch.utils.data.Dataset):
    def __init__(self, imgs_root, data_depth_velodyne_root, calib_velo2cam, calib_cam2cam, transforms=None, n_samples=None):
        ...
        source_imgs, data_velo_files, lidar_files = get_file_lists(
            imgs_root, data_depth_velodyne_root, n_samples = n_samples)
    ...
    def __getitem__(self, index):
        ...
        if img_filename.split("/")[-1].split(".")[0] != lidar_filename.split("/")[-1].split(".")[0]:
        ...
        return img, lidar_fov, mask, sparse_depth, k_nn_indices, img_name
...
def get_datasets(imgs_root, data_depth_velodyne_root, calib_velo2cam, calib_cam2cam, split=False, val_size=0.20, n_samples=None):

    kitti_dataset = kittiDataset(imgs_root=imgs_root, data_depth_velodyne_root=data_depth_velodyne_root, calib_velo2cam=calib_velo2cam, calib_cam2cam=calib_cam2cam, transforms=get_transform, n_samples=n_samples)
    if split:
        if val_size >= 1:
...
def get_dataloaders(batch_size, imgs_root, data_depth_velodyne_root, calib_velo2cam, calib_cam2cam, split=False, val_size=0.20, n_samples=None):
    ...
    # else:
    dataset = get_datasets(imgs_root=imgs_root, data_depth_velodyne_root=data_depth_velodyne_root, calib_velo2cam=calib_velo2cam, calib_cam2cam=calib_cam2cam, n_samples=n_samples)
