...
class myOwnDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        dummy_calib = "000114_calib.txt"
        calib_file = os.path.join(self.lidar_root, dummy_calib)
        calib = read_calib_file(calib_file)
        calib = project_velo_to_cam2(calib)
        calib = torch.tensor(calib, dtype=torch.float)

        dummy_lidar = "000114.bin"
        lidar_path = os.path.join(self.lidar_root, dummy_lidar)
        lidar_data = load_velo_scan(lidar_path)[:, :3]
        lidar_data = torch.tensor(lidar_data, dtype=torch.float)

        my_annotation["lidar_img"] = lidar_img
        my_annotation["calib"] = calib
        my_annotation["lidar_data"] = lidar_data
        ...
        return img, my_annotation, lidar_img, lidar_data, calib
