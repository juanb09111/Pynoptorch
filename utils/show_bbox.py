...
def show_bbox(img, preds, confidence, colors, file_name):
    boxes = preds['boxes']
    scores = preds['scores']
    labels = preds['labels']
    ...
    for i, box in enumerate(boxes):
        if scores[i] > confidence and labels[i] > 0 and labels[i] != 3:
            sample_box = [int(x) for x in box]
            width = sample_box[2] - sample_box[0]
            height = sample_box[3] - sample_box[1]
            xy= list([sample_box[0], sample_box[1]])
            edgecolor = colors[labels[i]]
            rect = patches.Rectangle(xy, width, height, linewidth=1,edgecolor=edgecolor,facecolor='none')
            ax.add_patch(rect)
            ax.text(xy[0], xy[1], categories[labels[i] - 1], style='italic',
            bbox={'facecolor': 'white', 'alpha': 0.5, 'pad': 1})
    plt.axis('off')
    plt.show()
    
    fig.savefig(os.path.join(my_path, '../results_mask/{}.png'.format(file_name)))
...
def show_masks(img, preds, confidence, colors, file_name):
    ...
    fig.savefig(os.path.join(my_path, '../maskRCNN_results_mask/{}.png'.format(file_name)))
...
def overlay_masks(img, preds, confidence, colors, folder, file_name):
    ...
    plt.axis('off')
    fig.savefig(os.path.join(my_path, '../{}/{}.png'.format(folder, file_name)))
    plt.close(fig)